...
import handler from '../../../server/api/tasks/[id]/comments/index.post'
...
const mockCreateCommentNotification = vi.hoisted(() => vi.fn())
const mockGetTaskDetails = vi.hoisted(() => vi.fn())
vi.mock('../../../server/utils/notificationService', () => ({
  createCommentNotification: mockCreateCommentNotification,
...
describe('POST /api/tasks/[id]/comments', () => {
  ...
  beforeEach(async () => {
    ...
    mockReadBody.mockResolvedValue({ content: 'Great comment' })
    mockGetVisibleStaffIds.mockReset()
    mockGetVisibleStaffIds.mockResolvedValue([1])
    mockGetTaskDetails.mockReset()
    mockGetTaskDetails.mockResolvedValue(null)
    mockCreateCommentNotification.mockReset()
  ...
  it('should return 404 when task is not found', async () => {
    supabase = createSupabaseMock({
      tasks: createResult(null, new Error('not found')),
    })
  ...
  it('should return 403 when no visible departments are available', async () => {
    supabase = createSupabaseMock({
      ...
      task_assignees: createResult([{ assigned_to_staff_id: 99 }]),
    })
  ...
  it('should return 403 when no assignee is in visible departments', async () => {
    ...
    mockGetVisibleStaffIds.mockResolvedValue([99])
  ...
  it('should create a comment and notify active assignees', async () => {
    const commentResponse = createResult({
      id: 100,
      task_id: 42,
      staff_id: 10,
    ...
    supabase = createSupabaseMock({
      ...
      staff: createResult({ id: 10, fullname: 'Alice', department: 'Engineering' }),
      task_assignees: [
        createResult([{ assigned_to_staff_id: 55 }]),
        createResult([
          { assigned_to_staff_id: 55 },
          { assigned_to_staff_id: 99 },
        ]),
      ],
      task_comments: commentResponse,
    })

    mockGetVisibleStaffIds.mockResolvedValue([55, 99])
    ...
    expect(mockCreateCommentNotification).toHaveBeenCalledTimes(2)
    expect(mockCreateCommentNotification).toHaveBeenCalledWith(
      expect.any(Object),
      42,
      55,
      10,
  ...
  it('should return 500 when inserting comment fails', async () => {
    supabase = createSupabaseMock({
      tasks: createResult({ id: 42, creator_id: 7 }),
      staff: createResult({ id: 10, fullname: 'Alice', department: 'Engineering' }),
      task_assignees: createResult([{ assigned_to_staff_id: 55 }]),
      task_comments: createResult(null, new Error('insert failed')),
    })
