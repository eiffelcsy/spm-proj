  ...
  <div class="w-full mx-auto p-8 md:px-12 lg:max-w-6xl xl:max-w-7xl">
    ...
    <div class="mb-4">
      ...
      >
        <!-- Modal content -->
        <div
          class="relative bg-white rounded-xl shadow-lg w-full max-w-2xl p-6 overflow-y-auto z-10 flex flex-col h-[65vh]"
          @click.stop
        >
          ...
          <!-- Feedback Messages -->
          <div
            v-if="projectSuccessMessage"
            class="flex-1 flex items-center justify-center"
          >
            <div
              class="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 text-green-800 rounded-xl p-8 flex flex-col items-center justify-center gap-6 min-w-[320px] min-h-[140px] shadow-lg"
            >
              ...
              <Button
                variant="outline"
                @click="handleProjectSuccessOk"
                class="bg-white hover:bg-green-50 border-green-300 text-green-700 hover:text-green-800 px-6 py-2"
                >OK</Button
              >
            </div>
          </div>
...
import { CalendarIcon, TriangleAlert } from "lucide-vue-next";
...
const error = computed(() => {
  if (fetchError.value)
    return (
      "Failed to load tasks. Please try again. " + fetchError.value.message
    );
  if (fetchOverdueError.value)
    return "Failed to load overdue tasks. Please try again.";
  return null;
});
