  ...
  <div class="w-full mx-auto p-8 md:px-12 lg:max-w-5xl xl:max-w-7xl">
    ...
    <div class="mb-4">
      ...
      <div v-if="isLoadingAny" class="flex justify-center items-center py-8">
        <div class="text-gray-500">Loading project tasks...</div>
      </div>
        ...
        <div v-if="overdueTasks.length > 0" class="mb-6">
          <div class="border-l-4 border-red-500 pl-4 mb-4 bg-red-50/70 p-4 rounded-r-lg">
            <h3 class="text-lg font-semibold pb-2 text-red-600 flex items-center">
              ...
              Overdue Project Tasks ({{ overdueTasks.length }})
            </h3>
            <DataTable 
...
const projectId = route.params.projectId || 'default' // Adjust as needed

const isModalOpen = ref(false)

// Fetch project tasks
const { data: tasksResponse, pending: isLoading, error: fetchError, refresh: refreshTasks } = await useFetch<{ tasks: any[], count: number }>(`/api/projects/${projectId}/tasks`)

// Fetch overdue project tasks
const { data: overdueTasksResponse, pending: isLoadingOverdue, error: fetchOverdueError, refresh: refreshOverdueTasks } = await useFetch<{ tasks: any[], count: number }>(`/api/projects/${projectId}/tasks/overdue`)
...
const error = computed(() => {
  if (fetchError.value) return 'Failed to load project tasks. Please try again. ' + fetchError.value.message
  if (fetchOverdueError.value) return 'Failed to load overdue project tasks. Please try again.'
  return null
})
